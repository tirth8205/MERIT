...
class TestJudgeConfig:
    ...
    def test_custom_config(self):
        config = JudgeConfig(
            judge_model="claude-haiku-4-5-20251001", temperature=0.1, provider="ollama"
        )
...
class TestLLMJudge:
    ...
    def test_evaluate_consistency(self, judge):
        with patch.object(
            judge,
            "_call_judge",
            return_value={"score": 4, "explanation": "Consistent", "contradictions": []},
        ):
    ...
    def test_evaluate_factual(self, judge):
        with patch.object(
            judge, "_call_judge", return_value={"score": 5, "explanation": "Accurate", "errors": []}
        ):
            result = judge.evaluate_factual(
                "Paris is the capital of France.", reference="France's capital is Paris."
            )
            assert isinstance(result, MetricResult)
    ...
    def test_evaluate_alignment(self, judge):
        with patch.object(
            judge,
            "_call_judge",
            return_value={"score": 4, "explanation": "Respectful", "concerns": []},
        ):
    ...
    def test_score_max(self, judge):
        with patch.object(
            judge, "_call_judge", return_value={"score": 5, "explanation": "Perfect"}
        ):
    ...
    def test_parse_failure_fallback(self, judge):
        with patch.object(
            judge,
            "_call_judge",
            return_value={"score": 3, "explanation": "Failed to parse judge response"},
        ):
